...
<marquee>Selamat datang di Aplikasi Parametrik Jitu, silahkan untuk menginput, memproses dan menganalisis data Anda, Terima Kasih</marquee>
	...
	<div id="menu">
       ...
       <table border="1"><tr><td><center>NO</center></td><td><center>Data Satu Variabel</center></td><td><center>Hapus</center></td></tr>
		<?php
    //file kelas.php
	include "db_connect.php";
    $query = mysql_query("select nama from tabel1 group by nama"); // data digroup jadi yang sama akan tampil satu
	$no=1;
    while($kelas = mysql_fetch_array($query)){
    	echo '<tr><td>'.$no.'</td><td><a href="tampil1.php?nama='.$kelas['nama'].'">'.$kelas['nama'].'</a></td><td><a href="delete.php?nama='.$kelas["nama"].'">delete</a></td></tr>';
		$no++;
    }?></table>
